...
- `jev-guard` is a PreToolUse hook: Jev checks the user's own transcript messages before `delete_all` and `delete_parameter` (never grants permission, only `ask`/`deny`)
